...
def detect_sags(pts, sag_drop=0.05):
    """Return indices of local elevation minima (sag points)."""
    return [i for i in range(1, len(pts) - 1)
            if pts[i][2] < pts[i-1][2] - sag_drop
            and pts[i][2] < pts[i+1][2] - sag_drop]


def ridge_sag_split_pts(pts, ridge_rise=0.05, sag_drop=0.05):
    """
    Split pts at ridges AND sags.
    Returns list of (sub_pts, start_type, end_type)
    where type is 'normal' | 'ridge' | 'sag'.
    """
    ri = set(detect_ridges(pts, ridge_rise))
    si = set(detect_sags(pts, sag_drop))
    split_indices = sorted(ri | si)

    if not split_indices:
        return [(pts, 'normal', 'normal')]

    bounds = [0] + split_indices + [len(pts) - 1]
    result = []
    for k in range(len(bounds) - 1):
        ...
        if len(sub) < 2:
            continue
        start_type = 'ridge' if a in ri else ('sag' if a in si else 'normal')
        end_type   = 'ridge' if b in ri else ('sag' if b in si else 'normal')
        result.append((sub, start_type, end_type))
    return result


def ridge_sag_split(noded_segs, elev_at, spacing=2.0,
                    ridge_rise=0.05, sag_drop=0.05):
    """
    Sample elevation, split at ridges and sags.
    Returns list of (pts_list, LineString, start_type, end_type) tuples.
    """
    result = []
    for seg in noded_segs:
        ...
        if len(pts) < 2:
            continue
        for sub_pts, st, et in ridge_sag_split_pts(pts, ridge_rise, sag_drop):
            if len(sub_pts) < 2:
                continue
            coords = [(p[0], p[1]) for p in sub_pts]
            result.append((sub_pts, LineString(coords), st, et))
    return result
...
def snap_outfalls_to_road_graph(of_pts, seg_tuples, snap_r=30.0, elev_at=None):
    ...
    for of_id, ox, oy in of_pts:
        ...
        if min_line_dist <= ROAD_ON_TOL and best_idx is not None:
            best_item = seg_list[best_idx]
            pts_orig, geom_orig = best_item[0], best_item[1]
            ...
            pieces_with_types = [(p[0], p[1], 'normal', 'normal') for p in pieces]
            seg_list[best_idx:best_idx + 1] = pieces_with_types
...
def assign_to_catchments(seg_tuples, catchments):
    ...
    assigned = []
    for item in seg_tuples:
        pts, geom = item[0], item[1]
        start_type = item[2] if len(item) > 2 else 'normal'
        end_type   = item[3] if len(item) > 3 else 'normal'
        matched = False
        for of_id, poly in zip(of_ids, cat_polys):
            ...
            if abs(inter.length - geom.length) < 0.01:
                assigned.append({
                    'pts': pts, 'geom': geom, 'territory': of_id,
                    'start_node_type': start_type, 'end_node_type': end_type,
                })
                matched = True
                break
            ...
            if inter.geom_type in ("LineString", "MultiLineString"):
                ...
                for piece in pieces:
                    ...
                    assigned.append({
                        'pts': piece_pts, 'geom': piece, 'territory': of_id,
        ...
        if not matched:
            assigned.append({
                'pts': pts, 'geom': geom, 'territory': None,
                'start_node_type': start_type, 'end_node_type': end_type,
            })
